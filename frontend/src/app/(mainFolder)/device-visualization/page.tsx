    ...
    axios
      ...
      .then((response) => {
        if (response.data.success && Array.isArray(response.data.data)) {
          ...
          setMetrics(types);
          setSelectedMetric(types[0] ? types[0].toLowerCase() : null);
          setComparisonMetric(types[1] ? types[1].toLowerCase() : null); // Default to second metric if available
          if (types[0]) {
            const found = response.data.data.find((t: any) => t.type.toLowerCase() === types[0].toLowerCase());
            setMetricLimits(found ? { min: found.minValue, max: found.maxValue } : null);
          } else {
  ...
  return (
    <div className="dashboard-container p-6 mt-20">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <Select value={selectedMetric ?? undefined} onValueChange={handleMetricChange} disabled={metrics.length === 0}>
          <SelectTrigger className="w-[200px]">
            <SelectValue placeholder="Select Metric" />
          </SelectTrigger>
          <SelectContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Card className="mb-6">
            ...
            <CardContent>
              ...
              {metrics.length > 1 && selectedMetric && (
                <div className="mb-2 flex items-center gap-2">
                  <span>Compare {selectedMetric} with:</span>
                  <Select
                    value={comparisonMetric ?? undefined}
                  ...
                  >
                    ...
                    <SelectContent>
                      {metrics.filter((m) => m.toLowerCase() !== selectedMetric.toLowerCase()).map((m) => (
              ...
              <CorrelationChart
                ...
                yType={comparisonMetric ?? undefined}
              />
            </CardContent>
          </Card>
          {/* Forecast (moved here, replacing Prediction) */}
          ...
          <Card className="mb-6">
            ...
            <CardContent>
              {devices.length >= 2 && devices[0] && devices[1] ? (
                <ComparisonChart
                  deviceA={devices[0].deviceId ?? undefined}
                  deviceAName={devices[0].deviceName ?? undefined}
                  deviceB={devices[1].deviceId ?? undefined}
                  deviceBName={devices[1].deviceName ?? undefined}
                  metric={selectedMetric ?? undefined}
                  startDateA={new Date(Date.now() - (timeRange === "lastHour" ? 60 * 60 * 1000 : 24 * 60 * 60 * 1000)).toISOString()}
                  endDateA={new Date().toISOString()}
                  startDateB={new Date(Date.now() - (timeRange === "lastHour" ? 60 * 60 * 1000 : 24 * 60 * 60 * 1000)).toISOString()}
