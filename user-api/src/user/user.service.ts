...
export class UserService {
  ...
  async findOrCreateUser(email: string, name: string): Promise<User> {
    ...
    try {
      ...
      if (existingUser) {
        console.log(
          `Found user: email=${normalizedEmail}, login_count=${existingUser.login_count}, user_id=${existingUser.user_id}`,
        );
        const updateData: any = {
          updated_at: new Date(),
          last_login: new Date(),
        ...
        console.log(
          `Updated user: email=${normalizedEmail}, login_count=${updatedUser.login_count}, user_id=${updatedUser.user_id}`,
        );
        return updatedUser;
      }
      ...
      const savedUser = await newUser.save();
      console.log(
        `Created user: email=${normalizedEmail}, user_id=${savedUser.user_id}`,
      );
      return savedUser;
    } catch (error) {
      console.error(
  ...
  async incrementLoginCount(email: string): Promise<User> {
    ...
    try {
      ...
      console.log(
        `User after increment: email=${normalizedEmail}, login_count=${user.login_count}, user_id=${user.user_id}`,
      );
      return user;
    } catch (error) {
      console.error(
