...
//       // Retry up to 3 times with 500ms delay to handle Auth0 script delay
//       for (let attempt = 1; attempt <= 3; attempt++) {
//         user = await this.userModel.findOne({ email }).exec();
//         if (user) {
//           console.log(
//             `Found user for ${email} on attempt ${attempt}: login_count = ${user.login_count}`,
//           );
//           return user;
//         }
//         console.log(
//           `User not found for ${email} on attempt ${attempt}, waiting 500ms`,
//         );
//         await new Promise((resolve) => setTimeout(resolve, 500));
//       }

//       // Create user if not found after retries
//       console.log(`Creating new user for ${email}`);
//       user = new this.userModel({
//         email,
//         name,
//         user_id: `manual|${email}`,
//         login_count: 0,
//         created_at: new Date(),
//         updated_at: new Date(),
//         last_login: new Date(),
...
//       }
//       console.log(`User after increment: login_count = ${user.login_count}`);
//       return user;
//     } catch (error) {
...
//       }
//       const user = await this.userModel
//         .findOneAndUpdate({ email }, { $set: update }, { new: true })
//         .exec();
//       if (!user) {
//         throw new Error(`User with email ${email} not found`);
//       }
