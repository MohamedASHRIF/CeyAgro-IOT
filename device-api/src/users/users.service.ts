...
export class UsersService {
  ...
  async updateFcmToken(email: string, fcmToken: string | null): Promise<void> {
    console.log(`Updating FCM token for user ${email}:`, fcmToken);
    const result = await this.userModel
      .findOneAndUpdate(
        { userId: email }, // Use email as userId
        { userId: email, fcmToken },
        { upsert: true, new: true },
      )
      .exec();
    console.log('MongoDB update result:', result);
  ...
  async getFcmToken(email: string): Promise<string | null> {
    const user = await this.userModel.findOne({ userId: email }).exec();
    return user?.fcmToken || null;
  }
}
