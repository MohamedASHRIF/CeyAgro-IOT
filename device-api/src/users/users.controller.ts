...
export class UsersController {
  ...
  async updateFcmToken(
    ...
    try {
      ...
      console.log(
        `Received request to update FCM token for user: ${decodedUserId}, token: ${body.fcmToken}`,
      );
      if (
        !body.fcmToken ||
        typeof body.fcmToken !== 'string' ||
        body.fcmToken.length < 100
      ) {
      ...
      console.error(
        `Error in updateFcmToken for user ${decodeURIComponent(userId)}:`,
        error,
      );
