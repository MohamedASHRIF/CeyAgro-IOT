  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleJoin(client: Socket, userId: string) {
    if (!userId || typeof userId !== 'string') {
      this.logger.error(`Invalid userId provided for join: ${userId}`);
      client.emit('error', { message: 'Invalid userId' });
      return;
    }
  ...
  emitNotification(notification: NotificationInterface) {
    const userId = notification.userId;
    if (!userId) {
      this.logger.error('Cannot emit notification: userId is missing');
      return;
    }
  ...
  emitNotificationRead(notification: NotificationInterface) {
    const userId = notification.userId;
    if (!userId) {
      this.logger.error('Cannot emit notificationRead: userId is missing');
      return;
    }

    this.logger.log(
      `Emitting notificationRead to user ${userId}: ${JSON.stringify(notification)}`,
    );
    this.server.to(userId).emit('notificationRead', notification);
  }
  ...
  emitNotificationDeleted(notificationId: string, userId?: string) {
    if (!userId) {
      this.logger.warn(
        `Emitting notificationDeleted to all clients: ${notificationId}`,
      );
      this.server.emit('notificationDeleted', notificationId);
      return;
    }
    ...
    this.logger.log(
      `Emitting notificationDeleted to user ${userId}: ${notificationId}`,
    );
    this.server.to(userId).emit('notificationDeleted', notificationId);
  }
